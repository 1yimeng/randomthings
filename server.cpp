...
vector<Point> pathFind(int start, int end, 
	...
	/*
	This function finds the path between two points.

	Argument: start (int for location index), end (int for location 
	index), tree (unordered_map<int, pair<int, long long> >),
	points (unordered_map<int, Point>).
	
	Return: a vector that has the points in sorted order
	*/
 	vector<Point> path;
